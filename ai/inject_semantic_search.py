#!/usr/bin/env python3
"""
Script to inject semantic_search_documents handler into rag_query.py
"""
...
def main():
    ...
    if 'elif tool_name == "semantic_search_documents":' in content:
        print("Semantic search handler already exists in rag_query.py")
        return

    # Find the insertion point
    marker = '    elif tool_name == "web_search_procurement":'
    if marker not in content:
        print(f"ERROR: Could not find marker: {marker}")
        return

    # Insert the handler
    content = content.replace(marker, SEMANTIC_HANDLER + '\n' + marker)

    # Write back
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)

    print("✓ Successfully injected semantic_search_documents handler into rag_query.py")
    print("✓ Handler added before web_search_procurement")
